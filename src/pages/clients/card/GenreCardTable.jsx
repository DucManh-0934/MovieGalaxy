...
import { FaTimes, FaPlay, FaRegClock } from "react-icons/fa";
import { convertDescription } from "../../../services/reponsitory";

function GenreCardTable({ category, gradient, movies = [], onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`relative px-6 py-5 bg-gradient-to-br ${gradient} flex items-center justify-between shrink-0`}>
          <div>
            <p className="text-white/80 text-xs uppercase tracking-[0.2em] font-semibold">Thể loại</p>
            <h2 className="text-white text-3xl font-bold">{category.name}</h2>
            {category.description && (
              <p className="text-white/70 text-sm mt-1 max-w-md">{category.description}</p>
            )}
          </div>
          <button
            onClick={onClose}
              ...
              <div className="p-3">
                ...
                {m.duration && (
                  <div className="flex items-center gap-1 mt-2 text-xs text-white/60">
                    <FaRegClock size={10} /> {m.duration} phút
                  </div>
                )}
              </div>
            </div>
